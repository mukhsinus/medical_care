...
(async () => {
  ...
  // export the running bot instance so other modules can reuse it
  try {
    module.exports = bot;
  } catch (err) {
    console.error('Failed to export bot instance:', err);
  }
  ...
  bot.onText(/\/login/, async (msg) => {
    ...
    if (existing) return bot.sendMessage(chatId, notifier.t(lang, 'already_logged_in'));
  ...
  bot.on('message', async (msg) => {
    ...
    if (state.state === LOGIN_STATES.WAITING_PASSWORD) {
      ...
      // Create and export bot synchronously so other modules can require it safely
      const bot = new TelegramBot(TOKEN, { polling: true });
      module.exports = bot;

      // Async initialization (DB connect, handlers registration)
      (async () => {
        await mongoose.connect(MONGO_URI);
        console.log('Bot connected to MongoDB');
        ...
        try {
          ...
          console.warn('Failed to delete webhook (may be fine):', err?.message || err);
        }
        ...
        bot.onText(/\/login/, async (msg) => {
          // require notifier lazily to avoid circular top-level dependency
          const notifier = require('./utils/telegramNotifier');
        ...
        bot.on('message', async (msg) => {
          ...
          if (state.state === LOGIN_STATES.WAITING_PASSWORD) {
            ...
            loginStates.delete(chatId);
            return bot.sendMessage(chatId, notifier.t(state.lang, 'logged_in', state.username));
          }
        });
        ...
        bot.onText(/\/clients/, async (msg) => {
          const chatId = msg.chat.id.toString();
          if (!(await BotSession.findOne({ chatId, expiresAt: { $gt: Date.now() } }))) return;
